...
export default function SettingsPanel({
  ...
  async function toggleEnabled() {
    ...
    // On iOS the schedules live with the system, so a prefs change has to be
    // pushed out — the app can't just consult them on the next tick.
    onPrefsChanged?.()
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      // Success means the auth listener is about to unmount all of this;
  ...
  return (
    <Modal onClose={onClose} wide>
          ...
          <>
            ...
            {blocked && (
              <p className="text-xs text-danger mt-2">
                {isNative()
                  ? 'Notifications are turned off for Lines. Allow them in Settings → Lines → Notifications.'
                  : "Notifications are blocked for this site. You'll need to re-allow them in your browser's site settings."}
              </p>
            )}
            ...
            <p className="text-xs text-faint mt-3 leading-relaxed">
              {isNative()
                ? 'Reminders are scheduled with iOS up to a week ahead, so they arrive even with the app closed. Each time you open Lines the week is re-planned from your latest changes — if you don\u2019t open it for over a week, reminders pause until you do.'
                : "These are local reminders, so they can only fire while Lines is open or was recently open in the background. They won't reach you if the app has been closed for a long time — that needs push notifications from a server, which this app doesn't run. Installing it to your home screen makes them noticeably more reliable."}
            </p>
          </>
        )}
      </section>
      ...
      {/* Account --------------------------------------------------------- */}
      <section className="pt-4 mt-4 border-t border-hairline">
        <h3 className="text-xs font-medium text-muted mb-2">Account</h3>
        {!confirmingDelete ? (
          <button
            onClick={() => setConfirmingDelete(true)}
            className="rounded-lg border border-hairlineStrong px-3 py-2.5 text-left w-full hover:border-danger hover:text-danger transition-colors"
          >
            <span className="flex items-center gap-1.5 text-sm text-ink">
              <AlertTriangle size={14} /> Delete my account
            </span>
            <span className="block text-xs text-muted mt-0.5">
              Permanently removes your account and every line, task and capture in it
            </span>
          </button>
        ) : (
          <div className="rounded-lg border border-dangerBorder bg-dangerSoft px-3 py-3">
            <p className="text-sm text-ink mb-1.5">This is permanent.</p>
            <p className="text-xs text-muted leading-relaxed mb-3">
              Your account and everything in it — every line, task, dependency, link and inbox
              capture — will be deleted from the server. There is no undo and no grace period.
              Consider exporting a JSON backup above first.
            </p>
            <label htmlFor="confirm-delete" className="block text-xs text-muted mb-1">
            ...
            <input
              ...
              autoComplete="off"
              className="w-full rounded-lg border border-hairlineStrong px-2.5 py-1.5 text-sm text-ink bg-panel focus:border-danger outline-none mb-2"
            />
            {deleteError && <p className="text-xs text-danger mb-2">{deleteError}</p>}
            {!online && (
              <p className="text-xs text-danger mb-2">
                You're offline — deletion needs a connection to reach the server.
              </p>
            )}
            <div className="flex gap-2">
              <button
                onClick={handleDelete}
                disabled={deleteText.trim().toLowerCase() !== 'delete' || deleting || !online}
                className="flex-1 rounded-lg bg-danger text-panel text-sm font-medium px-3 py-2 disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={deleting}
                className="rounded-lg border border-hairlineStrong text-sm px-3 py-2 text-ink"
              >
                Keep it
              </button>
            </div>
