...
The same code ships as a native iOS app through Capacitor (the `ios/` folder).
`npm run ios` builds the web assets, syncs them into the Xcode project and opens
it. On iOS, reminders become *scheduled* local notifications — planned a week
ahead and delivered by the system with the app closed — Sign in with Apple uses
the native sheet, and account deletion is available in Settings (an App Store
requirement, backed by the `delete-account` edge function: deploy it once with
`supabase functions deploy delete-account`). The full submission checklist lives
